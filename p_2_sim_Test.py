...
#from sim_functions.Repeat_check import Repeating

def fault_Vectors_Creation():
#-----------------------#puts the Tv into an 2d array ---------------------------#
#          0|1|2|3|4|5|.........
#       A|
#       B|
#       C|
...
####################################################################################
    user_seed = input("\nInsert your seed: ")
    ArrayA=[]
    ArrayA = TV_A(user_seed)
    ArrayB = TV_B(user_seed)
    ArrayC = TV_C(user_seed)
    # ArrayD = lfsr_D(user_seed)    dont run
    # ArrayE = lfsr_E(user_seed)    dont run
    rows,cols = (5,255) # 5 rows and each row will have 255 tv's
    TV_Array = [[0]*cols]*rows # generates an empty 2-d array

    TV_Array[0]= ArrayA # tr position 0 we have all 255 tv's from A
    TV_Array[1]= ArrayB# at position 0 we have all 255 tv's from B
    TV_Array[2]= ArrayC# ...
    print(TV_Array)

    #---------------Code used from fault Generation Goal is to find total number of faults!--------#
    ...
    #pprint(faultList)
    Total_faults = len(faultList)
    print( " Total faults in this circuit: ", Total_faults)
#----------------------------------#we found the total number of faults#--------------------#

# ----------------------------------next step run T.V. Generation for each value 0-255 for A-B--------#
# TV[0][0]->TV[0][255]-> put first results into an array then check if the value is in it for each run. 255 runs X total faults in worse case give 1000 faults =255k units of time??
# TV[1][0]->TV[1][255]
...
#[-5:]
    ...
    if userSimulationChoice.strip() == "0":
        ...
        faultListGeneration()
        return
    elif userSimulationChoice.strip() == "2":
        fault_Vectors_Creation()
        return
